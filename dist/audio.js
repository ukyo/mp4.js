...
exports.createMp4DescriptorSpecificInfo = createMp4DescriptorSpecificInfo;
...
function extractAudioAsAAC(bytes, audioTrack) {
...
function extractAudioAsMP3(bytes, audioTrack) {
    return helpers_1.concatBytes(helpers_1.getChunks(bytes, audioTrack));
}
function extractRawAudio(bytes) {
    var tree = helpers_1.parse(bytes);
    var audioTrack = getAudioTrack(tree);
    var finder = new finder_1.Finder(audioTrack);
    var mp4a = (finder.findOne(statics_1.BOX_TYPE_MP4_AUDIO_SAMPLE_ENTRY));
...
exports.extractRawAudio = extractRawAudio;
function aacToM4a(bytes) {
    ...
    var decConfigDescr = {
        ...
        avgBitrate: 0,
        decSpecificInfo: {
            data: createMp4DescriptorSpecificInfo(sampleRate, aacInfo.channelConfiguration)
        }
    };
    var slConfigDescr = {
        preDefined: 2
    ...
    var mdatBytes = new composer_box_1.MediaDataBoxBuilder({
        data: helpers_1.concatBytes(samples)
    }).build();
    return helpers_1.concatBytes([ftypBytes, moovBytes, mdatBytes]);
}
exports.aacToM4a = aacToM4a;
