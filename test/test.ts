...
import {
  ...
  ISyncSampleBox,
  IMediaDataBox
} from "../src/interface.box";
import { m4v } from "./init-m4v-bytes";
import { xml } from "./init-xml";
import $ from "jquery";
...
describe("Parser", () => {
  ...
  describe("FileTypeBox", () => {
    ...
    var ftyp = <IFileTypeBox>finder.findOne("ftyp");
    var $info = $ftyp.find("BoxInfo");

    it("size", () => expect(ftyp.byteLength).toBe(+$info.attr("Size")));
    it("major brand", () =>
      expect(ftyp.majorBrand).toBe($ftyp.attr("MajorBrand")));
    it("minor version", () =>
      expect(ftyp.minorVersion).toBe(+$ftyp.attr("MinorVersion")));
  ...
  describe("MovieBox", () => {
    ...
    var moov = <IMovieBox>finder.findOne("moov");
    ...
    describe("MovieHeaderBox", () => {
      ...
      var mvhd = <IMovieHeaderBox>finder.findOne("mvhd");
      var $info = $mvhd.find("BoxInfo");
      var $fullinfo = $mvhd.find("FullBoxInfo");

      it("size", () => expect(mvhd.byteLength).toBe(+$info.attr("Size")));
      it("version", () =>
        expect(mvhd.version).toBe(+$fullinfo.attr("Version")));
      it("flags", () => expect(mvhd.flags).toBe(+$fullinfo.attr("Flags")));
      it("creation time", () =>
        expect(mvhd.creationTime).toBe(+$mvhd.attr("CreationTime")));
      it("modification time", () =>
        expect(mvhd.modificationTime).toBe(+$mvhd.attr("ModificationTime")));
      it("time scale", () =>
        expect(mvhd.timescale).toBe(+$mvhd.attr("TimeScale")));
      it("duration", () => expect(mvhd.duration).toBe(+$mvhd.attr("Duration")));
      it("next track id", () =>
        expect(mvhd.nextTrackID).toBe(+$mvhd.attr("NextTrackID")));
    });

    describe("TrackBox 1(Audio Track)", () => {
      var $trak = $moov.find("TrackBox:nth(0)");
      var trak = <ITrackBox>finder.findOne("trak");
      ...
      describe("TrackHeaderBox", () => {
        ...
        var tkhd = <ITrackHeaderBox>finder.findOne("tkhd");
        var $info = $tkhd.find("BoxInfo");
        var $fullinfo = $tkhd.find("FullBoxInfo");

        it("size", () => expect(tkhd.byteLength).toBe(+$info.attr("Size")));
        it("version", () =>
          expect(tkhd.version).toBe(+$fullinfo.attr("Version")));
        it("flags", () => expect(tkhd.flags).toBe(+$fullinfo.attr("Flags")));
        it("creation time", () =>
          expect(tkhd.creationTime).toBe(+$tkhd.attr("CreationTime")));
        it("modifiction time", () =>
          expect(tkhd.modificationTime).toBe(+$tkhd.attr("ModificationTime")));
        it("track id", () => expect(tkhd.trackID).toBe(+$tkhd.attr("TrackID")));
        it("duration", () =>
          expect(tkhd.duration).toBe(+$tkhd.attr("Duration")));
      ...
      describe("MediaBox", () => {
        ...
        var mdia = <IMediaBox>finder.findOne("mdia");
        ...
        describe("MediaHeaderBox", () => {
          ...
          var mdhd = <IMediaHeaderBox>finder.findOne("mdhd");
          var $info = $mdhd.find("BoxInfo");
          var $fullinfo = $mdhd.find("FullBoxInfo");

          it("size", () => expect(mdhd.byteLength).toBe(+$info.attr("Size")));
          it("version", () =>
            expect(mdhd.version).toBe(+$fullinfo.attr("Version")));
          it("flags", () => expect(mdhd.flags).toBe(+$fullinfo.attr("Flags")));
          it("creation time", () =>
            expect(mdhd.creationTime).toBe(+$mdhd.attr("CreationTime")));
          it("modification time", () =>
            expect(mdhd.modificationTime).toBe(
              +$mdhd.attr("ModificationTime")
            ));
          it("timescale", () =>
            expect(mdhd.timescale).toBe(+$mdhd.attr("TimeScale")));
          it("duration", () =>
            expect(mdhd.duration).toBe(+$mdhd.attr("Duration")));
          it("language", () =>
            expect(mdhd.language).toBe($mdhd.attr("LanguageCode")));
        });

        describe("HandlerBox", () => {
          var $hdlr = $mdia.find("HandlerBox");
          var hdlr = <IHandlerBox>finder.findOne("hdlr");
          var $info = $hdlr.find("BoxInfo");
          var $fullinfo = $hdlr.find("FullBoxInfo");

          it("size", () => expect(hdlr.byteLength).toBe(+$info.attr("Size")));
          it("version", () =>
            expect(hdlr.version).toBe(+$fullinfo.attr("Version")));
          it("flags", () => expect(hdlr.flags).toBe(+$fullinfo.attr("Flags")));
          it("handler type", () =>
            expect(hdlr.handlerType).toBe($hdlr.attr("Type")));
          it("name", () => expect(hdlr.name).toBe($hdlr.attr("Name")));
        });

        describe("MediaInformationBox", () => {
          var $minf = $mdia.find("MediaInformationBox");
          var minf = <IMediaInformationBox>finder.findOne("minf");
          ...
          describe("SoundMediaHeaderBox", () => {
            ...
            var smhd = <ISoundMediaHeaderBox>finder.findOne("smhd");
            var $info = $smhd.find("BoxInfo");
            var $fullinfo = $smhd.find("FullBoxInfo");

            it("size", () => expect(smhd.byteLength).toBe(+$info.attr("Size")));
            it("version", () =>
              expect(smhd.version).toBe(+$fullinfo.attr("Version")));
            it("flags", () =>
              expect(smhd.flags).toBe(+$fullinfo.attr("Flags")));
          });

          describe("DataInformationBox", () => {
            var $dinf = $minf.find("DataInformationBox");
            var dinf = <IDataInformationBox>finder.findOne("dinf");
            ...
            describe("DataReferenceBox", () => {
              ...
              var dref = <IDataReferenceBox>finder.findOne("dref");
              var $info = $dref.find("BoxInfo");
              var $fullinfo = $dref.find("FullBoxInfo");

              it("size", () =>
                expect(dref.byteLength).toBe(+$info.attr("Size")));
              it("version", () =>
                expect(dref.version).toBe(+$fullinfo.attr("Version")));
              it("flags", () =>
                expect(dref.flags).toBe(+$fullinfo.attr("Flags")));

              describe("URLDataEntryBox", () => {
                var $url = $dref.find("URLDataEntryBox");
                var url = <IDataEntryUrlBox>finder.findOne("url ");
                var $info = $url.find("BoxInfo");
                var $fullinfo = $url.find("FullBoxInfo");

                it("size", () =>
                  expect(url.byteLength).toBe(+$info.attr("Size")));
                it("version", () =>
                  expect(url.version).toBe(+$fullinfo.attr("Version")));
                it("flags", () =>
                  expect(url.flags).toBe(+$fullinfo.attr("Flags")));
              });
            });
          });
          ...
          describe("SampleTableBox", () => {
            ...
            var stbl = <ISampleTableBox>finder.findOne("stbl");
            ...
            describe("SampleDescriptionBox", () => {
              ...
              var stsd = <ISampleDescriptionBox>finder.findOne("stsd");
              var $info = $stsd.find("BoxInfo");
              var $fullinfo = $stsd.find("FullBoxInfo");

              it("size", () =>
                expect(stsd.byteLength).toBe(+$info.attr("Size")));
              it("version", () =>
                expect(stsd.version).toBe(+$fullinfo.attr("Version")));
              it("flags", () =>
                expect(stsd.flags).toBe(+$fullinfo.attr("Flags")));
              ...
              describe("MP4AudioSampleEntry", () => {
                ...
                it("size", () =>
                  expect(mp4a.byteLength).toBe(+$info.attr("Size")));
                it("data reference index", () =>
                  expect(mp4a.dataReferenceIndex).toBe(
                    +$mp4a.attr("DataReferenceIndex")
                  ));
                it("sample rate", () =>
                  expect(mp4a.sampleRate).toBe(+$mp4a.attr("SampleRate")));
                it("channel count", () =>
                  expect(mp4a.channelCount).toBe(+$mp4a.attr("Channels")));
                it("sample size", () =>
                  expect(mp4a.sampleSize).toBe(+$mp4a.attr("BitsPerSample")));
                ...
                describe("ESDescriptor", () => {
                  ...
                  it("es ID", () =>
                    expect(esDescr.esID).toBe(+$esDescr.attr("ES_ID")[2]));

                  describe("DecoderConfigDescriptor", () => {
                    var $decConfigDescr = $esDescr.find(
                      "DecoderConfigDescriptor"
                    );
                    var decConfigDescr = esDescr.decConfigDescr;

                    it("object type indication", () =>
                      expect(decConfigDescr.objectTypeIndication).toBe(
                        +$decConfigDescr.attr("objectTypeIndication")
                      ));
                    it("stream type", () =>
                      expect(decConfigDescr.streamType).toBe(
                        +$decConfigDescr.attr("streamType")
                      ));
                    it("buffer size DB", () =>
                      expect(decConfigDescr.bufferSizeDB).toBe(
                        +$decConfigDescr.attr("bufferSizeDB")
                      ));
                    it("max bitrate", () =>
                      expect(decConfigDescr.maxBitrate).toBe(
                        +$decConfigDescr.attr("maxBitrate")
                      ));
                    it("avg bitrate", () =>
                      expect(decConfigDescr.avgBitrate).toBe(
                        +$decConfigDescr.attr("avgBitrate")
                      ));

                    describe("DecoderSpecificInfo", () => {
                      var $decSpecificInfo = $decConfigDescr.find(
                        "DecoderSpecificInfo"
                      );
                      ...
                      it("data", () => {
                        $decSpecificInfo
                          ...
                          .match(/\d+/g)
                          .forEach((x, i) => {
                            expect(decSpecificInfo.data[i]).toBe(
                              parseInt(x, 16)
                            );
                          });
                      });
                    ...
                    describe("SLConfigDescriptor", () => {
                      ...
                      it("pre defined", () =>
                        expect(slConfigDescr.preDefined).toBe(
                          +$slConfigDescr.find("predefined").attr("value")
                        ));
                    });
                  });
                });
            ...
            describe("TimeToSampleBox", () => {
              ...
              var stts = <ITimeToSampleBox>finder.findOne("stts");
              var $info = $stts.find("BoxInfo");
              var $fullinfo = $stts.find("FullBoxInfo");

              it("size", () =>
                expect(stts.byteLength).toBe(+$info.attr("Size")));
              it("version", () =>
                expect(stts.version).toBe(+$fullinfo.attr("Version")));
              it("flags", () =>
                expect(stts.flags).toBe(+$fullinfo.attr("Flags")));
              it("entries", () => {
                $stts.find("TimeToSampleEntry").map((i, el) => {
                  var $el = $(el);
                  expect(stts.entries[i].sampleCount).toBe(
                    +$el.attr("SampleCount")
                  );
                  expect(stts.entries[i].sampleDelta).toBe(
                    +$el.attr("SampleDelta")
                  );
                });
              });
            ...
            describe("SampleToChunkBox", () => {
              ...
              it("size", () =>
                expect(stsc.byteLength).toBe(+$info.attr("Size")));
              it("version", () =>
                expect(stsc.version).toBe(+$fullinfo.attr("Version")));
              it("flags", () =>
                expect(stsc.flags).toBe(+$fullinfo.attr("Flags")));
              it("entries", () => {
                $stsc.find("SampleToChunkEntry").map((i, el) => {
                  var $el = $(el);
                  expect(stsc.entries[i].firstChunk).toBe(
                    +$el.attr("FirstChunk")
                  );
                  expect(stsc.entries[i].samplesPerChunk).toBe(
                    +$el.attr("SamplesPerChunk")
                  );
                  expect(stsc.entries[i].sampleDescriptionIndex).toBe(
                    +$el.attr("SampleDescriptionIndex")
                  );
                });
              });
            });
            ...
            describe("SampleSizeBox", () => {
              ...
              it("size", () =>
                expect(stsz.byteLength).toBe(+$info.attr("Size")));
              it("version", () =>
                expect(stsz.version).toBe(+$fullinfo.attr("Version")));
              it("flags", () =>
                expect(stsz.flags).toBe(+$fullinfo.attr("Flags")));
              it("sample sizes", () => {
            ...
            describe("ChunkOffsetBox", () => {
              ...
              it("size", () =>
                expect(stco.byteLength).toBe(+$info.attr("Size")));
              it("version", () =>
                expect(stco.version).toBe(+$fullinfo.attr("Version")));
              it("flags", () =>
                expect(stco.flags).toBe(+$fullinfo.attr("Flags")));
              it("chunk offsets", () => {
    ...
    describe("TrackBox 2(Video Track)", () => {
      ...
      var trak = <ITrackBox>finder.findAll("trak")[1];
      var $info = $trak.find("BoxInfo");
      ...
      describe("EditBox", () => {
        ...
        var edts = <IEditBox>finder.findOne("edts");
          ...
          */
          ...
          describe("SampleTableBox", () => {
            ...
            describe("SyncSampleBox", () => {
              ...
              var stss = <ISyncSampleBox>finder.findOne("stss");
              var $info = $stss.find("BoxInfo");
              var $fullinfo = $stss.find("FullBoxInfo");

              it("size", () =>
                expect(stss.byteLength).toBe(+$info.attr("Size")));
              it("version", () =>
                expect(stss.version).toBe(+$fullinfo.attr("Version")));
              it("flags", () =>
                expect(stss.flags).toBe(+$fullinfo.attr("Flags")));
              it("sample numbers", () => {
                $stss.find("SyncSampleEntry").map((i, el) => {
                  expect(stss.sampleNumbers[i]).toBe(
                    +$(el).attr("sampleNumber")
                  );
                });
              });
            });
  ...
  describe("MediaDataBox", () => {
    ...
    var mdat = <IMediaDataBox>finder.findOne("mdat");
...
describe("Builder", () => {
  ...
  describe("FileTypeBox", () => {
    var ftyp1 = <IFileTypeBox>finder.findOne(Mp4.BOX_TYPE_FILE_TYPE_BOX);
    ftyp1.bytes = null;
    ...
    it("minor version", () =>
      expect(ftyp2.minorVersion).toBe(ftyp2.minorVersion));
    it("compatible brands", () => {
  ...
  describe("MovieHeaderBox", () => {
    var mvhd1 = <IMovieHeaderBox>finder.findOne(Mp4.BOX_TYPE_MOVIE_HEADER_BOX);
    mvhd1.bytes = null;
    ...
    it("creation time", () =>
      expect(mvhd2.creationTime).toBe(mvhd1.creationTime));
    it("modification time", () =>
      expect(mvhd2.modificationTime).toBe(mvhd1.modificationTime));
    it("time scale", () => expect(mvhd2.timescale).toBe(mvhd1.timescale));
    it("duration", () => expect(mvhd2.duration).toBe(mvhd1.duration));
    it("next track id", () =>
      expect(mvhd2.nextTrackID).toBe(mvhd1.nextTrackID));
  });
  ...
  describe("TrackHeaderBox", () => {
    var tkhd1 = <ITrackHeaderBox>finder.findOne(Mp4.BOX_TYPE_TRACK_HEADER_BOX);
    tkhd1.bytes = null;
    ...
    it("creation time", () =>
      expect(tkhd2.creationTime).toBe(tkhd1.creationTime));
    it("modification time", () =>
      expect(tkhd2.modificationTime).toBe(tkhd1.modificationTime));
  ...
  describe("MediaHeaderBox", () => {
    var mdhd1 = <IMediaHeaderBox>finder.findOne(Mp4.BOX_TYPE_MEDIA_HEADER_BOX);
    mdhd1.bytes = null;
    ...
    it("creation time", () =>
      expect(mdhd2.creationTime).toBe(mdhd1.creationTime));
    it("modification time", () =>
      expect(mdhd2.modificationTime).toBe(mdhd1.creationTime));
  ...
  describe("HandlerBox", () => {
    var hdlr1 = <IHandlerBox>finder.findOne(Mp4.BOX_TYPE_HANDLER_BOX);
    hdlr1.bytes = null;
  ...
  describe("DataReferenceBox", () => {
    var dref1 = <IDataReferenceBox>(
      finder.findOne(Mp4.BOX_TYPE_DATA_REFERENCE_BOX)
    );
    dref1.bytes = null;
    ...
    describe("DataEntryUrlBox", () => {
      var url1 = <IDataEntryUrlBox>dref1.entries[0];
      url1.bytes = null;
  ...
  describe("SampleDescriptionBox", () => {
    var stsd1 = <ISampleDescriptionBox>(
      finder.findOne(Mp4.BOX_TYPE_SAMPLE_DESCRIPTION_BOX)
    );
    stsd1.bytes = null;
    ...
    describe("MP4AudioSampleEntry", () => {
      var mp4a1 = <IMP4AudioSampleEntry>(
        finder.findOne(Mp4.BOX_TYPE_MP4_AUDIO_SAMPLE_ENTRY)
      );
      mp4a1.bytes = null;
      ...
      it("data reference index", () =>
        expect(mp4a2.dataReferenceIndex).toBe(mp4a1.dataReferenceIndex));
      ...
      describe("ESDescriptor", () => {
        ...
        esDescr1.bytes = null;
        var esDescrBytes = new Mp4.Builder.ESDescriptorBuilder(
          esDescr1
        ).build();
        ...
        describe("DecoderConfigDescriptor", () => {
          ...
          var decConfigDescrBytes = new Mp4.Builder.DecoderConfigDescriptorBuilder(
            decConfigDescr1
          ).build();
          var decConfigDescr2 = new Mp4.Parser.DecoderConfigDescriptorParser(
            decConfigDescrBytes
          ).parse();

          it("object type indication", () =>
            expect(decConfigDescr2.objectTypeIndication).toBe(
              decConfigDescr1.objectTypeIndication
            ));
          it("stream type", () =>
            expect(decConfigDescr2.streamType).toBe(
              decConfigDescr1.streamType
            ));
          it("buffer size DB", () =>
            expect(decConfigDescr2.bufferSizeDB).toBe(
              decConfigDescr1.bufferSizeDB
            ));
          it("max bitrate", () =>
            expect(decConfigDescr2.maxBitrate).toBe(
              decConfigDescr1.maxBitrate
            ));
          it("avg bitrate", () =>
            expect(decConfigDescr2.avgBitrate).toBe(
              decConfigDescr1.avgBitrate
            ));

          describe("DecoderSpecificInfo", () => {
            var decSpecificInfo1 = decConfigDescr1.decSpecificInfo;
            decSpecificInfo1.bytes = null;
            var decSpecificInfoBytes = new Mp4.Builder.DecoderSpecificInfoBuilder(
              decSpecificInfo1
            ).build();
            var decSpecificInfo2 = new Mp4.Parser.DecoderSpecificInfoParser(
              decSpecificInfoBytes
            ).parse();

            it("data", () => {
              expect(decSpecificInfo2.data.length).toBe(
                decSpecificInfo1.data.length
              );
        ...
        describe("SLConfigDescriptor", () => {
          ...
          var slConfigDescrBytes = new Mp4.Builder.SLConfigDescriptorBuilder(
            slConfigDescr1
          ).build();
          var slConfigDescr2 = new Mp4.Parser.SLConfigDescriptorParser(
            slConfigDescrBytes
          ).parse();

          it("pre defined", () =>
            expect(slConfigDescr2.preDefined).toBe(slConfigDescr1.preDefined));
        });
      });
    });
  });
  ...
  describe("TimeToSampleBox", () => {
    var stts1 = <ITimeToSampleBox>(
      finder.findOne(Mp4.BOX_TYPE_TIME_TO_SAMPLE_BOX)
    );
    stts1.bytes = null;
  ...
  describe("SampleToChunkBox", () => {
    var stsc1 = <ISampleToChunkBox>(
      finder.findOne(Mp4.BOX_TYPE_SAMPLE_TO_CHUNK_BOX)
    );
    stsc1.bytes = null;
  ...
  describe("SampleSizeBox", () => {
    var stsz1 = <ISampleSizeBox>finder.findOne(Mp4.BOX_TYPE_SAMPLE_SIZE_BOX);
    stsz1.bytes = null;
  ...
  describe("ChunkOffsetBox", () => {
    var stco1 = <IChunkOffsetBox>finder.findOne(Mp4.BOX_TYPE_CHUNK_OFFSET_BOX);
    stco1.bytes = null;
